...
class Jcart {
	...
	*/
	public function display_cart() {
		...
		echo tab(7) . "<span id='jcart-subtotal'>{$config['text']['subtotal']}: <strong>RM " . number_format($this->subtotal, $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "</strong></span>\n";
		...
		if ($isCheckout !== true) {
			...
			echo tab(8) . "\n<input type='$inputType' $src id='jcart-checkout' name='jcartCheckout' class='btn danger' value='View Cart' />\n";
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
				...
				echo tab(7) . "<input class=\"mini\" id='jcartItemQty-{$item['id']}' name='jcartItemQty[]' size=\"2\" type=\"text\" value='{$item['qty']}' />\n";
				echo tab(6) . "</td>\n";
				...
				echo tab(7) . "<span>RM" . number_format($item['subtotal'], $priceFormat['decimals'], $priceFormat['dec_point'], $priceFormat['thousands_sep']) . "</span><input name='jcartItemPrice[]' type='hidden' value='{$item['price']}' />\n";
		...
		echo tab(4) . "<input type='$inputType' $src name='jcartUpdateCart' value='{$config['text']['update']}' class='btn info' />\n";
		...
		echo tab(4) . "<input type='$inputType' $src name='jcartEmpty' value='{$config['text']['emptyButton']}' class='btn info' />\n";
		echo tab(3) . "</div>\n";
		...
		if ($isCheckout === true) {
			...
			echo tab(3) . "<input type='hidden' id='jcart-is-checkout' name='jcartIsCheckout' value='true' class='btn danger'/>\n";

			// PayPal checkout button
			if ($config['button']['checkout'])	{
				$inputType = "image";
				$src = " src='{$config['button']['checkout']}' alt='{$config['text']['checkoutPaypal']}' title='' class='btn large danger'";
			}
			...
			echo tab(3) . "<input type='$inputType' $src id='jcart-paypal-checkout' name='jcartPaypalCheckout' value='Check Out' $disablePaypalCheckout class='btn large danger'/>\n";
		}
	...
	* Process and display cart
	*/
	public function display_final_cart() {
		...
		// Display the cart header
		echo tab(1) . "$errorMessage\n";
		echo tab(1) . "<form method='post' action='finalaction.php'";
		echo tab(2) . "<fieldset>\n";
		echo tab(3) . "<input type='hidden' name='jcartToken' value='{$_SESSION['jcartToken']}' />\n";
		echo tab(3) . "<table border='1'>\n";
		echo tab(4) . "<thead>\n";
		...
		echo tab(4) . "<input type=\"hidden\" value=\"$this->subtotal\" name=\"total\" />";
		echo tab(4) . "</tbody>\n";
		echo tab(3) . "</table>\n\n";
		...
		if ($isCheckout !== true) {
			...
			echo tab(8) . "<td><input type='$inputType' $src id='jcart-checkout' name='jcartCheckout' class='btn danger large' value='Check Out' /></td>\n";
